...
public:
	...
	// overloaded method. Uses deques instead of vectors. No other differences.
	void exec(const char *cmd, std::deque<std::string> &terminal_output, std::deque<std::string> &error_list, bool realtime_terminal_feedback);
};
...
void command::exec(const char* cmd, std::deque<std::string> &terminal_output, std::deque<std::string> &error_list, bool realtime_terminal_feedback) {
    char buffer[128];
    FILE* pipe = popen(cmd, "r");
    if (!pipe) {
        error_list.push_back("popen() failed!");
